...
import { IFDataProductOrigin } from '@src/domain/models/IProductOrigin'
...
const ModalComponent = () => {
  ...
  const { openModal, handleModal, dataItem, typeModal } = useContext(ProductOriginContext)
  // ** States
  const getWindowSize = () => {
  ...
  const formSchema = yup.object().shape({
    code: yup
      .string()
      .required((t("Product origin code") + t(" is required")))
      .matches(/^[A-Za-z0-9]+$/, t("Do not contain special characters"))
      .max(50, (t("Product origin code") + t(" must less than 255 characters"))),
    name: yup.string()
      .required((t("Product origin name") + t(" is required")))
      .max(255, t("Product origin name") + t(" must less than 255 characters"))
      .min(2, (t("Product origin name") + t(" must greater than 2 characters"))),
    description: yup
      .string()
      .max(500, (t("Description") + t(" must less than 500 characters")))
  })

  const defaultValues: IFDataProductOrigin = {
    description: "",
    id: "",
    code: "",
    name: "",
  ...
  const onSubmit = (data: any) => {
    if (typeModal === 'Add') {
      addProductOriginApi(data).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
      ...
      editProductOriginApi(data).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
  ...
  return (
    ...
    >
      <Form id='form-modal-globalzone' className='todo-modal' onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader handleModal={handleModal} typeModal={typeModal} title='Product origin' />
        <Scrollbars autoHide
          autoHeight
