...
import { IFDataWard } from '@src/domain/models/IWard'
...
const ModalComponent = () => {
  ...
  const { openModal, handleSidebar, dataItem, type } = useContext(WardContext)
  // ** States
  const getWindowSize = () => {
  ...
  const defaultValues: IFDataWard = {
    note: "",
    id: "",
    code: "",
    name: "",
  ...
  const handleFormOpened = () => {
    if (!isObjEmpty(dataItem)) {
      ...
      setValue("shortName", dataItem.shortName)
      setValue("note", dataItem.note)
      setValue("status", dataItem.status)
      setValue("districtId", dataItem.districtId)
    } else {
      setValue("code", "")
      setValue("name", "")
      setValue("shortName", "")
  ...
  const dataToRender = () => {
    getListDistrictApi({ $status: 1 }).unwrap()
      .then((rs) => {
        setTimeout(() => {
                    ...
                    <div className='form-input-content'>
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <Select
                            value={optionDistrict.find((val: any) => val.value === value)}
                  ...
                  <div className='d-flex form-row-inline label-small'>
                    ...
                    <div className='form-input-content'>
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <Select
                            value={optionStatus.find((val: any) => val.value === value)}
                            onChange={(val: any) => onChange(val.value)}
                            placeholder={t("Select")}
                            classNamePrefix="select"
                            className='react-select'
