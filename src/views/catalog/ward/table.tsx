...
import { useDistrict } from "../district/hooks"
...
const FilterHeader = ({
  ...
  optionCountry,
  handleDistrictAssignedToChange,
      ...
      <Row className="m-0 gx-1 d-flex flex-wrap justify-content-end align-items-center w-100">
        ...
        <Col xs={12} sm={6} md={3} lg={2} className="pt-25 pb-25">
          <Select
            ...
            value={districtAssignedTo}
            onChange={handleDistrictAssignedToChange}
            placeholder={t("Select Option")}
            className="react-select"
            classNamePrefix="select"
          ></Select>
...
const Table = () => {
  const { t } = useTranslation()
  const { handleSidebar, setDataItem, setTypeModal } = useContext(WardContext)
  // ** States
  ...
  const [districtAssignedTo, setDistrictAssignedTo] = useState<any>({
    value: "",
    label: t("All district")
  })
  ...
  // ** Effect
  const { getWardPagingApi, deleteWardApi, checkInit } = useWard()

  const { getListCountryApi } = useCountry()

  const { getListDistrictApi } = useDistrict()
  ...
  useEffect(() => {
    getListDistrictApi(
      stateProvinceAssignedTo.value ? stateProvinceAssignedTo.value : ""
    )
      .unwrap()
      .then((rs) => {
        rs.splice(0, 0, { value: "", label: t("All district") })
        setTimeout(() => {
          setOptionDistrict(rs)
        }, 10)
      })
      .catch((ex) => {
        console.log(ex)
  ...
  const dataToRender = () => {
    getWardPagingApi({
      ...
      $parentId: districtAssignedTo.value
    })
      .unwrap()
      .then((rs) => {
        setTimeout(() => {
  ...
  const handleDistrictAssignedToChange = (val: any) => {
    setDistrictAssignedTo(val)
  }
        ...
        <Card className="table-bar bar__query">
          <CardBody className="pe-0 ps-0">
            <FilterHeader
              ...
              handleDistrictAssignedToChange={handleDistrictAssignedToChange}
              handleStateProvinceAssignedToChange={
                handleStateProvinceAssignedToChange
              }
              ...
              districtAssignedTo={districtAssignedTo}
