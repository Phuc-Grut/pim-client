...
import { Modal, ModalBody, Button, Form, Label, Row, Col, Input } from 'reactstrap'
...
import { IFDataProductCategory } from '@src/domain/models/IProductCategory'
...
const ModalComponent = () => {
  ...
  const { setParentCategory, openModal, handleModal, setDataItem, dataItem, typeModal, groupAssignedTo, parentCategory, breadCrumb } = useContext(ProductCategoryContext)
  // ** States
  const getWindowSize = () => {
  ...
  const formSchema = yup.object().shape({
    code: yup
      ...
      .min(2, (t("CategoryProductCode") + t(" must greater than 2 characters"))),
    name: yup.string()
  ...
  const defaultValues: IFDataProductCategory = {
    id: "",
    code: "",
    name: "",
    description: "",
  ...
  const onSubmit = (data: any) => {
    ...
    if (typeModal === 'Add') {
      addProductCategoryApi(obj).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
      ...
      editProductCategoryApi(obj).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
  ...
  return (
    ...
    >
      <Form id='form-modal-globalzone' className='todo-modal' onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader handleModal={handleModal} typeModal={typeModal} title="CategoryProduct" />
        <Scrollbars autoHide
          autoHeight
