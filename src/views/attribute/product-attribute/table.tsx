...
import {canContextMenuItems} from "@src/views/attribute/product-attribute/contextMenu"
  ...
  return (
    <div className="d-flex justify-content-between align-items-center w-100">
      <div className="bar__action-left">
        <BreadCrumbsNotLink breadCrumbActive={t("Product attribute")} breadCrumbParent={{ title: t('Attribute') }}/>
      </div>
      <div className="bar__action-right d-flex">
        <CanPer I={userAction.ADD} a={userSubject["Product attribute"]}>
...
const Table = () => {
  const { t } = useTranslation()
  const { handleModal, setDataItem, setTypeModal, handleModalDetail, setDataTableDetail, windowSize } = useContext(
  ...
  const listKey = ["status", "allowFiltering"]
  const listDate: any = ['']
  ...
  const handleModalSort = () => {
    getListProductAttributeApi({}).unwrap()
      .then((rs) => {
        setDataSort(rs)
  ...
  const optionAllowFiltering: any = [
    {
      value: true,
      label: `${t("Yes")}`
    },
  ...
  return (
    <Fragment>
      <div className="table-toolbar">
        ...
        <GridTableTemplate
          resource="tableProductAttributes"
          idTable="ProductAttributes"
        ...
        <ModalSortComponent
          ...
          setData={setDataSort}
          columns={sortColumns}
          sortApi={sortProductAttributeApi}
        />
      </div>
    </Fragment>
  )
