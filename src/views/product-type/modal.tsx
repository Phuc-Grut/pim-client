...
import { IFDataProductType } from '@src/domain/models/IProductType'
...
const ModalComponent = () => {
  ...
  const { openModal, handleModal, dataItem, typeModal } = useContext(ProductTypeContext)
  // ** States
  const getWindowSize = () => {
  ...
  const formSchema = yup.object().shape({
    code: yup
      .string()
      .required((t("Product type code") + t(" is required")))
      .matches(/^[A-Za-z0-9]+$/, t("Do not contain special characters"))
      .max(50, (t("Product type code") + t(" must less than 255 characters"))),
    name: yup.string()
      .required((t("Product type name") + t(" is required")))
      .max(255, t("Product type name") + t(" must less than 255 characters"))
      .min(2, (t("Product type name") + t(" must greater than 2 characters"))),
    description: yup
      .string()
      .max(500, (t("Description") + t(" must less than 500 characters")))
  })

  const defaultValues: IFDataProductType = {
    description: "",
    id: "",
    code: "",
    name: "",
  ...
  const onSubmit = (data: any) => {
    if (typeModal === 'Add') {
      ...
      editProductTypeApi(data).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
  ...
  return (
    ...
    >
      <Form id='form-modal-globalzone' className='todo-modal' onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader handleModal={handleModal} typeModal={typeModal} title='Product type' />
        <Scrollbars autoHide
          autoHeight
