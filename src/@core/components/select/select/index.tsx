...
const SelectBox = (props: IFSelectBox) => {
  ...
  const { control, placeholder, disabled, name, options, label, isLabel, labelSize, classes, required, errors, isMulti, isClearable, labelComponent, callback, inLine, ...rest } = props
  const renderSelect = () => {
    return (
      <Fragment>
        <Controller
          ...
          render={({ field: { value, onChange } }) => {
            return <Select
              {...rest}
              value={(isMulti && !isNullOrUndefined(value) && value?.length > 0) ? (value.map((x: any) => options?.find((e: any) => e.value === x))) : (!isNullOrUndefined(value) && value !== '') ? options?.find((val: any) => val.value === value) : ''}
              //value={(!isNullOrUndefined(value) && value !== '') ? options.find((val: any) => val.value === value) : ''}
              onChange={(val: any) => {
                if (isMulti) {
                  onChange((!isNullOrUndefined(val) && isMulti) ? val.map((item: any) => item.value) : ((!isNullOrUndefined(val) && !isMulti) ? val.value : undefined))
                  if (callback) {
                    callback(val)
                  }
                } else {
